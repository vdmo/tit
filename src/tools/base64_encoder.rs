...
impl<'a> Base64Encoder<'a> {
    ...
    fn process(&mut self) {
        ...
        self.output = match self.mode {
            Mode::Encode => STANDARD.encode(&text),
            Mode::Decode => {
                let text = text.replace(&['\n', '\r', ' '][..], "");
                match STANDARD.decode(&text) {
                    Ok(bytes) => String::from_utf8(bytes).unwrap_or_else(|_| "Invalid UTF-8 sequence".to_string()),
                    Err(_) => "Invalid Base64 sequence".to_string(),
                }
            }
        };
    }
